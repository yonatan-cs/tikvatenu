...
export function AdminSidebar({ locale, userName }: Props) {
  ...
  return (
    <>
      ...
      <button
        className="md:hidden fixed top-4 start-4 z-[110] w-11 h-11 flex items-center justify-center bg-white rounded-lg shadow-md border border-branch/10"
      ...
      <aside
        className={`
          md:hidden fixed inset-y-0 start-0 z-[100] w-64 bg-white h-[100dvh] shadow-2xl flex flex-col
          transform transition-transform duration-300 ease-in-out
          ${mobileOpen ? "translate-x-0" : "-translate-x-full"}
        `}
      >
        <div className="flex items-center justify-between p-4 pt-6 border-b border-branch/10">
          <div>
            <p className="text-sm font-medium text-navy truncate">{userName}</p>
            <p className="text-xs text-ink-muted">{isHebrew ? "מנהל" : "Administrator"}</p>
          </div>
          <button
            onClick={() => setMobileOpen(false)}
            className="p-2 text-ink-light hover:text-error hover:bg-error/5 rounded-lg transition-colors"
